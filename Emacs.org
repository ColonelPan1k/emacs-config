* Credits
A lot of the inspiration for this has been taken from [[https://github.com/hrs/dotfiles/tree/main/emacs][here]] and [[https://github.com/daviwil/emacs-from-scratch][here]].
* Startup
** Use-package
...
I can't stand case-sensitive completion, get rid of it.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Backups
The Emacs =.#= files are annoying, messy, and get in the way
of things.  I want them gone.
...
#+END_SRC
** Set Default Browser
When I open a link in emacs, I'd like it to use eww rather than
firefox.
#+BEGIN_SRC emacs-lisp
...
* User Interface
** General UI Config
Clean up the UI and make it look more minimal

#+BEGIN_SRC emacs-lisp
  (setq inhibit-startup-message t
	initial-buffer-choice nil)  ; Disable the startup message
  ...
  (setq ring-bell-function 'ignore) ; Disable the windows bell
  (setq visible-bell t)             ; Enable the visible bell
  (setq global-hl-line-mode t)      ; Enable global line highlighting
#+END_SRC
...
And load the current theme that I'm using. Suscolors and Seventies are also two
other themes I've used extensively in the past.

#+BEGIN_SRC emacs-lisp
  (load-theme 'cyberpunk)
#+END_SRC

Cyberpunk theme has an odd issue with line highlighting and source blocks, though
it seems like it's only affecting this file.
** Font
*** Set the font
#+BEGIN_SRC emacs-lisp
...
This file keeps track of what I've done at work each day
...
#+BEGIN_SRC emacs-lisp
  ...
  (defconst org-todo-file "~/Org-Files/todo/todo.org")
...
Disable electric-pair-mode for '<' in org-mode.  This is useful for source blocks.
...
** Archiving
#+BEGIN_SRC emacs-lisp
  ; Taken from github.com/hrs/dotfiles
  ...
  (defun save-archive-file ()
    (interactive)
    (save-some-buffers 'no-confirm (lambda ()
				   (cond (equal buffer-file-name (expand-file-name org-archive-location))
					 (equal buffer-file-name (expand-file-name org-todo-file))))))
...
reading/learning about anything.
...
# Configure the buffers to display properly
# #+begin_src emacs-lisp
...
#+END_SRC
* Centaur Tabs
TODO: Group centaur tabs by file type, not directory
      e.g. all .py files should be grouped together
...
#+end_src
TODO: Move over zsh git aliases into magit
* Helm
#+begin_src emacs-lisp
  (use-package helm
    :config
